...
#define off_time   20 // время задерки выключения
// рассчитывается по формуле t * 0,1 сек.
// Т.е. 20 - ~2 секунды
#define Pwr_OK_time 500 // время, в мс, ожидания сигнала
// Pwr_OK при включении БП.
...
////////////////////////////////////////////////////////////
    do
    {
        ...
        delay_ms(Pwr_OK_time);                            // Задержка на включение БП
        ...
        do                                                // Цикл генерации сигнала ошибки
        {
            ...
            if (button != 0 && ptmr > off_time)           // кнопка нажата более 2 сек.
            {
                pstop = 0;
                PS_ON = 0;                                // вырубаем питание БП
            ...
            if (button == 0 && ptmr <= off_time) kn = 0;  // кнопка нажата менее 4 сек.
        }
        while (pstop);
        pstop = 1;
